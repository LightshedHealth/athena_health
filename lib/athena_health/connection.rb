...
module AthenaHealth
  class Connection
    ...
    AUTH_PATH   = { 'v1' => 'oauth2', 'openpreview1' => 'oauthopenpreview' }
    ...
    def authenticate
      response = Typhoeus.post(
        "#{@base_url}/#{AUTH_PATH[@version]}/v1/token",
        userpwd: "#{@key}:#{@secret}",
    ...
    def call(endpoint:, method:, params: {}, body: {}, second_call: false)
      ...
      if response.response_code != 200
        AthenaHealth::Error.new(code: response.response_code, message: body).render
      end
